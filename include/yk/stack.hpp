...
public:
    [[nodiscard]] friend constexpr bool operator==(stack const& lhs, stack const& rhs)
        noexcept(noexcept(static_cast<base_stack const&>(lhs) == static_cast<base_stack const&>(rhs)))
    {
...
stack(InputIt, InputIt, Alloc = Alloc())
    -> stack<std::iter_value_t<InputIt>, std::vector<std::iter_value_t<InputIt>, Alloc>>;
#endif
...
stack(std::from_range_t, R&&)
    -> stack<std::ranges::range_value_t<R>>;

// C++23
template<std::ranges::input_range R, xo::simple_allocator Alloc>
stack(std::from_range_t, R&&, Alloc)
    -> stack<std::ranges::range_value_t<R>, std::vector<std::ranges::range_value_t<R>, Alloc>>;
#endif
...
struct uses_allocator<::yk::stack<T, Container>, Alloc>
